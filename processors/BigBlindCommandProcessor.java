...
public class BigBlindCommandProcessor
  implements RequestCommandProcessor
{
  ...
  public Response process(HashMap params, Server server)
    throws IOException
  {
    Response response = new Response("BBLIND");
    if (server.getCurrentPlayer() != null) {
      if (params.containsKey("d"))
      {
        ...
        if (desk != null) {
          ...
          if (place != null) {
            if (desk.getGame().acceptStake(new Stake(8, place))) {
              place.getPlaceSessionStats().countTotalBettedForLastHour(desk.getMaxBet());
